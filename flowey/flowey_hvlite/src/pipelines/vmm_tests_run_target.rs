...
use flowey_lib_hvlite::_jobs::local_run_nextest_vmm_tests::validate_target_incubator_profile;
...
pub struct VmmTestsRunTargetCli {
    ...
    /// Run tests inside a QEMU TCG incubator.
    ///
    /// CCA profiles require host-bound platform artifacts. Use `vmm-tests-run`
    /// or its generated `run.sh` on the build host for CCA.
...
impl IntoPipeline for VmmTestsRunTargetCli {
    fn into_pipeline(self, backend_hint: PipelineBackendHint) -> anyhow::Result<Pipeline> {
        ...
        if let Some(profile) = &incubator_profile {
            validate_target_incubator_profile(profile, &dir)?;
        }
...
mod tests {
        ...
        for name in ["aarch64-fvp-cca", "aarch64-qemu-cca"] {
            for profile in [
                ...
                let error = cli
                    ...
                    .err()
                    .expect("CCA must not construct a target-run pipeline");
                assert!(
                    error
                        .to_string()
                        .contains("vmm-tests-run-target supports only QEMU TCG"),
                    "{error:#}"
                );
            }
        }
    ...
    #[test]
    fn target_run_preserves_native_and_tcg_pipelines() {
        let repo_root = crate::repo_root();
        for profile in [None, Some(""), Some("aarch64-tcg-pcie")] {
            let mut args = vec![
                "vmm-tests-run-target",
                "--target",
                "linux-aarch64-musl",
            ...
            assert!(cli.args.into_pipeline(PipelineBackendHint::Local).is_ok());
        }
    }
}
