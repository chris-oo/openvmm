...
/// Coherent CCA payload and host-platform artifacts resolved by the build job.
#[derive(Serialize, Deserialize)]
pub struct CcaTestArtifacts {
    pub realm_kernel: ReadVar<PathBuf>,
    pub host_kernel: ReadVar<PathBuf>,
...
flowey_request! {
    pub struct Params {
        ...
        /// Override the generic incubator payload and platform for CCA.
        pub cca_artifacts: Option<CcaTestArtifacts>,
...
impl SimpleFlowNode for Node {
    ...
    fn process_request(request: Self::Request, ctx: &mut NodeCtx<'_>) -> anyhow::Result<()> {
        ...
        anyhow::ensure!(
            cca_artifacts.is_none() || incubator_profile.is_some(),
            "CCA artifacts require an incubator profile"
        );
        ...
        let (
            ...
            TestContentConfig::Uninitialized {
                ...
                let initialized = ctx.reqv(|v| crate::init_vmm_tests_content_dir::Request {
                    ...
                    test_linux_kernel_override: cca_artifacts
                        .as_ref()
                        .map(|a| a.realm_kernel.clone()),
                    test_linux_initrd_override: cca_artifacts.as_ref().map(|a| a.initrd.clone()),
                    cca_payload_only: cca_artifacts
                        .as_ref()
                        .is_some_and(|a| a.fvp_roots.is_some()),
                    done: v,
                });
        ...
        let extra_env = if let Some(incubator_profile) = incubator_profile {
            ...
            let (kernel, initrd, firmware, qemu_binary, fvp_roots) = if let Some(artifacts) =
                cca_artifacts
            {
                (
                    artifacts.host_kernel,
                    artifacts.initrd,
                    artifacts.firmware,
                    artifacts.qemu_binary,
                    artifacts.fvp_roots,
                )
            } else {
                ...
                (kernel, initrd, None, Some(qemu_binary), None)
            };
