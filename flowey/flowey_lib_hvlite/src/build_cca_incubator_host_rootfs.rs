...
//! Build the CCA incubator L1 host rootfs with pipette startup.

use flowey::node::prelude::*;

#[derive(Serialize, Deserialize, Clone, Debug)]
pub struct CcaIncubatorHostRootfsBuildParams {
...
#[derive(Serialize, Deserialize, Clone, Debug)]
pub struct CcaIncubatorHostRootfsOutput {
    pub image: PathBuf,
    pub init_script: PathBuf,
    pub manifest: PathBuf,
}

impl Artifact for CcaIncubatorHostRootfsOutput {}

flowey_request! {
    pub struct Request {
        pub params: CcaIncubatorHostRootfsBuildParams,
        pub output: WriteVar<CcaIncubatorHostRootfsOutput>,
    }
}
        ...
        for Request { params, output } in requests {
            ctx.emit_rust_step("build QEMU CCA host rootfs", |ctx| {
                ...
                move |rt| {
                    ...
                    let script = params
                        .openvmm_root
                        .join("build_support/cca/build-incubator-host-rootfs.sh");
                    anyhow::ensure!(
                        script.is_file(),
                    ...
                    let artifact = CcaIncubatorHostRootfsOutput {
                        image: params.output_root.join("host-rootfs.ext4"),
                        init_script: params.output_root.join("incubator-host-init.sh"),
