...
pub fn build_zero_page(
    ...
    initrd_base: u32,
    initrd_size: u32,
    setup_header: Option<defs::setup_header>,
) -> defs::boot_params {
    let mut hdr = setup_header.unwrap_or_else(FromZeros::new_zeroed);
    hdr.type_of_loader = 0xff;
    hdr.boot_flag = 0xaa55.into();
    hdr.header = 0x53726448.into();
    ...
    if u32::from(hdr.kernel_alignment) == 0 {
        hdr.kernel_alignment = 0x100000.into();
    }
...
where
    F: Read + Seek,
{
    let load_info = match kernel_format {
        KernelFormat::Elf => load_kernel_and_initrd_x64(
            importer,
            kernel_image,
            kernel_minimum_start_address,
            initrd,
