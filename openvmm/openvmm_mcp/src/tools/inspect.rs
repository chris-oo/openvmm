...
pub fn tools() -> Vec<(ToolDefinition, Handler)> {
    vec![
       ...
       (
           ToolDefinition {
               name: "pv/inspect".into(),
               description:
                   "Inspect the paravisor's internal state via the diagnostic channel. \
                    This queries the OpenHCL paravisor running inside the VM, \
                    not the host-side VMM."
                       .into(),
               input_schema: serde_json::json!({
                   "type": "object",
                   "properties": {
                       "path": {
                           "type": "string",
                           "description": "Inspect path inside the paravisor (e.g. '' for root, 'serial-com1' for COM1 state)",
                           "default": ""
                       },
                       "depth": {
                           "type": "integer",
...
fn handle_pv_inspect(
   ...
   Box::pin(async move {
       ...
       let diag = match &vm.diag_client {
           Some(d) => d,
           None => return ToolResult::error("paravisor diagnostics not available"),
       };

       match diag
           .inspect(&path, Some(depth), Some(Duration::from_secs(5)))
           .await
       {
           Ok(node) => ToolResult::text(format!("{}", node.json())),
           Err(e) => ToolResult::error(format!("pv inspect failed: {e:#}")),
       }
   })
}
